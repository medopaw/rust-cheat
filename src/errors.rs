...
| 类型 | 用途 | 典型示例 |
|------|------|----------|
| `anyhow::Result` | 应用层统一错误处理 | CLI 工具、Web 服务 |
| `thiserror` | 库层自定义错误 | 组件、SDK、中间件 |
| `with_context()` | 添加调试上下文 | 文件读取、网络请求 |
| `?` 操作符 | 错误传播和转换 | 错误处理链 |
...
- 🔧 **看错误传播**: `?` 操作符自动转换和提前返回  
- ⚠️ **看上下文添加**: `with_context()` 提供调试信息

## ⚠️ AI 常见问题警告

> **危险信号** 🚨

- 🔴 **过度 unwrap** 到处使用 `unwrap()`/`expect()` 导致程序 panic
- 🔴 **信息不明** 错误信息不明确（"Something went wrong"）  
- 🔴 **策略混用** 混用不同错误处理策略（anyhow + 自定义错误）
- 🔴 **上下文缺失** 忽略错误上下文，难以定位问题源头
...
**Step 1: 错误类型识别**  
先看函数签名的错误类型（anyhow vs thiserror）
